...
class RPC_Server:
    def metrics(self, typ: int, symbol: str, country: str, api: int):
        api = API(api)
        api = findata.AlphaVantage() if api is API.ALPHA_VANTAGE else findata.Leeway() if api is API.LEEWAY else findata.FinancialModelingPrep() if api is API.FINANCIAL_MODELING_PREP else None
        doc = {}
